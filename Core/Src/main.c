...
// LTDC_HandleTypeDef hltdc; // Removed: Conflicts with hLtdcHandler from BSP
...
/* USER CODE BEGIN PV */
extern int lcd_io_putchar(int ch);

int __io_putchar(int ch)
{
  HAL_UART_Transmit(&huart1, (uint8_t *)&ch, 1, 0xFFFF);
  // lcd_io_putchar(ch); // Temporarily disabled to prevent hangs
  return ch;
}

#define MODEM_CHUNK_SIZE 64
typedef struct {
    float samples[MODEM_CHUNK_SIZE];
} ModemChunk;

int16_t audio_buffer[AUDIO_BUFFER_SIZE] __attribute__((aligned(32)));
int16_t playback_buffer[AUDIO_BUFFER_SIZE] __attribute__((aligned(32)));
...
int main(void)
{
  ...
  printf("PeriphCommonClock_Config OK\r\n");
  ...
  MX_GPIO_Init();
  printf("MX_GPIO_Init OK\r\n");
  MX_ADC3_Init();
  MX_CRC_Init();
  printf("MX_CRC_Init OK\r\n");
  MX_DCMI_Init();
  printf("MX_DCMI_Init OK\r\n");
  MX_DMA2D_Init();
  MX_ETH_Init();
  printf("MX_ETH_Init OK\r\n");
  // MX_FMC_Init();   // Handled by BSP_SDRAM
  MX_I2C1_Init();
  printf("MX_I2C1_Init OK\r\n");
  MX_I2C3_Init();
  printf("MX_I2C3_Init OK\r\n");
  // MX_LTDC_Init(); // Handled by BSP_LCD
  // MX_QUADSPI_Init();
  // printf("MX_QUADSPI_Init OK\r\n");
  MX_RTC_Init();
  printf("MX_RTC_Init OK\r\n");
  // MX_SAI2_Init(); // Handled by BSP_AUDIO_IN
  MX_SDMMC1_SD_Init();
  printf("MX_SDMMC1_SD_Init OK\r\n");
  MX_SPDIFRX_Init();
  printf("MX_SPDIFRX_Init OK\r\n");
  MX_SPI2_Init();
  ...
  MX_TIM12_Init();
  printf("TIMs Init OK\r\n");
  // MX_USART1_UART_Init(); // Moved earlier
  MX_USART6_UART_Init();
  printf("MX_USART6_UART_Init OK\r\n");
  MX_FATFS_Init();
  printf("MX_FATFS_Init OK\r\n");

  /* USER CODE BEGIN 2 */
  printf("\r\n--- STM32 DiscVu Booting ---\r\n");
  ...
  // CRITICAL: Use raw FreeRTOS queue for 256-byte ModemChunk objects. 
  // osMessageCreate is limited to 4-byte pointers/integers on this platform.
  modem_samples_queue_id = (osMessageQId)xQueueCreate(16, sizeof(ModemChunk));
  if (modem_samples_queue_id == NULL) printf("[ERROR] FSK Queue Create FAILED!\r\n");
  /* USER CODE END RTOS_QUEUES */

  /* Create the thread(s) */
  /* definition and creation of defaultTask */
  // Reduced stack from 4096 (16KB) to 1024 (4KB) to fit in 32KB Heap
  osThreadDef(defaultTask, StartDefaultTask, osPriorityNormal, 0, 1024);
  defaultTaskHandle = osThreadCreate(osThread(defaultTask), NULL);
  if (defaultTaskHandle == NULL) printf("[ERROR] Default Task Create FAILED (Out of Heap?)\r\n");
...
/* USER CODE BEGIN 4 */
static void Audio_Bridge_Process(int16_t* inBuf, uint32_t samples) {
    static ModemChunk currentChunk;
    static int chunkIdx = 0;
    static float fsk_dc = 0.0f;
    extern float g_FSKGain;
    
    // Invalidate D-Cache for the buffer we just received from DMA
    ...
    for (uint32_t i = 0; i < numModemSteps; i++) {
        ...
        float ganged = cleanAvg * g_FSKGain;
        if (ganged > 1.0f) ganged = 1.0f; else if (ganged < -1.0f) ganged = -1.0f;
        
        // Track max signal for debug (extern float g_MaxSignalLevel)
        extern float g_MaxSignalLevel;
        if (fabsf(ganged) > g_MaxSignalLevel) g_MaxSignalLevel = fabsf(ganged);

        currentChunk.samples[chunkIdx++] = ganged;
        
        if (chunkIdx >= MODEM_CHUNK_SIZE) {
            if (modem_samples_queue_id != NULL) {
                BaseType_t xHigherPriorityTaskWoken = pdFALSE;
                if (xQueueSendFromISR(modem_samples_queue_id, &currentChunk, &xHigherPriorityTaskWoken) != pdPASS) {
                    static uint32_t last_q_err = 0;
                    if (HAL_GetTick() - last_q_err > 1000) {
                        printf("[FSK] Queue FULL! Dropping samples.\r\n");
                        last_q_err = HAL_GetTick();
                    }
                }
...
// Modem Task: Process FSK Logic in high-priority background
extern FSK_Modem g_Modem;
extern bool g_ShowFSK;
extern void addFSKChar(char c);
...
void StartDefaultTask(void const * argument)
{
  ...
  // Initialize SDRAM first as in JMinimodem-Cube
  ...
  printf("[FATFS] Mounting SD card (delayed)...\r\n");
